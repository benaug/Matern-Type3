...
source("NimModel Matern3 V1b.R")
source("Nimble Functions Matern3 V1b.R")
source("mask.check.R")
...
Niminits <- list(N.primary=N.secondary,N.thin=0,D0=D0.init,r=r.init, #init r very low so starting logProb finite
...
# config.nodes <- c("D0","r","age1") #only need to supply updates for 1:N.secondary ages
#if using density covariate, use this and add block sampler for D0 and D.beta1 below
config.nodes <- c("r","age1") #only need to supply updates for 1:N.secondary ages
