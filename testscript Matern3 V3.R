...
source("NimModel Matern3 V3.R")
source("Nimble Functions Matern3 V3.R")
source("mask.check.R")
...
Niminits <- list(N.primary=sum(z.init),D0=D0.init,z=z.init,sigma.thin=sigma.thin,r=data$r,
...
# config.nodes <- c("D0","sigma.thin",paste("age[",1:data$N.secondary,"]")) #only need to supply updates for 1:N.secondary ages
#if using density covariate, use this and add block sampler for D0 and D.beta1 below
config.nodes <- c("sigma.thin",paste("age[",1:data$N.secondary,"]")) #only need to supply updates for 1:N.secondary ages
