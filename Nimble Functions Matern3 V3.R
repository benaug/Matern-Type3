...
dThin <- nimbleFunction(
  run = function(x=double(1),D=double(2),age=double(1),r=double(0),sigma.thin=double(0),
    ...
    for(i in 1:M){
      if(z[i]==1){
        ...
        if(n.points>0){
          ...
          kern <- exp(-(D[i,these.points2])^2 / sigma.thin^2)
...
rThin <- nimbleFunction(
  run = function(n=integer(0),D=double(2),age=double(1),r=double(0),sigma.thin=double(0),z=double(1)){
    returnType(double(1))
    M <- nimDim(D)[1]
    return(rep(0,M))
  }
  ...
  run = function(){
    ...
    for(i in 1:N.secondary){
      for(j in 1:N.prop){
        ...
        shadow2D[i,j] <- 1*(D.prop[i,j]<model$r[1]&model$age[i]<age.prop[j])*exp(-(D.prop[i,j])^2/model$sigma.thin[1]^2)
      }
    }
