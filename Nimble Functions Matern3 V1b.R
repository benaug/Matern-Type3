...
dThin <- nimbleFunction(
  run = function(x=double(1),D1=double(2),D.cross=double(2),
                 age1=double(1),age2=double(1),r=double(0),N.thin=double(0),log=integer(0)){
    returnType(double(0))
    N.secondary <- nimDim(D1)[1]
    logProb <- 0
    #retained-retained pairs - for hardcore, make sure no n.points>0&z==1 or n.points==0&z==0
    for(i in 1:N.secondary){
      n.points <- 0
      for(j in 1:N.secondary){
      ...
      if(n.points>0){ #cannot be retained if prior retained point in radius
        logProb <- -Inf
      }
    }
    ...
    if(N.thin>0 & logProb != -Inf){#don't need to check thinned points if logProb is -Inf from secondary points
      for(j in 1:N.thin){
        ...
        if(n.points==0){ #all thinned points must have prior secondary points
          logProb <- -Inf
        } #else n.points>0, valid
      }
    }
    return(logProb)
  }
...
rThin <- nimbleFunction(
  run = function(n=integer(0),D1=double(2),D.cross=double(2),
                 age1=double(1),age2=double(1),r=double(0),N.thin=double(0)){
    returnType(double(1))
    N.secondary <- nimDim(D1)[1]
...
thinnedSampler <- nimbleFunction(
  ...
  run = function(){
    ...
    for(i in 1:N.secondary){
      for(j in 1:N.prop){
        #points within r and later than focal secondary point
