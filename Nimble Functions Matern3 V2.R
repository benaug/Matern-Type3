...
dThin <- nimbleFunction(
  run = function(x=double(1),D=double(2),age=double(1),r=double(0),p.thin=double(0),
...
rThin <- nimbleFunction(
  run = function(n=integer(0),D=double(2),age=double(1),r=double(0),p.thin=double(0),z=double(1)){
    returnType(double(1))
    M <- nimDim(D)[1]
    return(rep(0,M))
  }
