...
source("NimModel Matern3 V2.R")
source("Nimble Functions Matern3 V2.R")
source("mask.check.R")
...
Niminits <- list(N.primary=sum(z.init),D0=D0.init,z=z.init,p.thin=0.5,r=data$r, #r often needs to be initialized pretty close to truth
...
config.nodes <- c("r","p.thin",paste("age[",1:data$N.secondary,"]")) #only need to supply updates for 1:N.secondary ages
conf <- configureMCMC(Rmodel,monitors=parameters, thin=nt,
                      monitors2=parameters2, thin2=nt2,
                      useConjugacy = FALSE, nodes=config.nodes) 
#add sampler for thinned points and ages
conf$addSampler(target = paste("s[1:",M,",1:2]"),
                type = 'thinnedSampler',control = list(M=M,N.secondary=data$N.secondary,n.cells.x=data$n.cells.x,n.cells.y=data$n.cells.y,
                                                       res=data$res),
                silent = TRUE)
#add block sampler for density model if using density covariate    
