...
dThin <- nimbleFunction(
  run = function(x=double(1),D1=double(2),D.cross=double(2),
                 age1=double(1),age2=double(1),r=double(0),sigma.thin=double(0),N.thin=double(0),log=integer(0)){
    returnType(double(0))
    ...
    for(i in 1:N.secondary){
      ...
      if(n.points>0){
        ...
        kern <- exp(-(D1[i,these.points2])^2 / sigma.thin^2)
        log.p.retain <- sum(log(1 - kern))
    ...
    if(N.thin>0 & logProb != -Inf){#don't need to check thinned points if logProb is -Inf from secondary points
      ...
      for(j in 1:N.thin){
        ...
        if(n.points>0){ 
          these.points2 <- these.points[1:n.points] #can't resize these.points
          kern <- exp(-(D.cross[these.points2,j])^2 / sigma.thin^2)
          log.p.retain <- sum(log(1 - kern))
...
rThin <- nimbleFunction(
  run = function(n=integer(0),D1=double(2),D.cross=double(2),
                 age1=double(1),age2=double(1),r=double(0),sigma.thin=double(0),N.thin=double(0)){
    returnType(double(1))
    N.secondary <- nimDim(D1)[1]
  ...
  run = function(){
    ...
    for(i in 1:N.secondary){
      for(j in 1:N.prop){
        ...
        shadow2D[i,j] <- 1*(D.prop[i,j]<model$r[1]&model$age1[i]<age2.prop[j])*exp(-(D.prop[i,j])^2/model$sigma.thin[1]^2)
      }
    }
